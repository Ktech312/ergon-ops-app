...
15. **§5 row 11 — share-link expiration/revocation — DONE AND VERIFIED (2026-09-13).** Decided in full via `PRODUCT_SHARE_LINK_EXPIRATION_REVOCATION_DECISION.md`; implemented and deployed as migrations 137 (+ corrective 141), 138, 139, 140 (+ corrective 142) — all applied and verified in production, including every canonical test — plus the Queue C2.6 internal lifecycle-controls UI (Disable/Re-enable, Permanently Revoke & Generate New Link, activity history). Remaining: Queue C2.7 (narrow the old direct-write bypasses on `public_share_tokens`/`sales_quote_proposals`/`project_submittals`, and switch the Create & Send flow to the new server-owned RPCs). *Verification: Migration (done, prod-verified), Code (done, prod-verified), User acceptance pending real use.*
...
| 11. Share-link expiration/revocation | **DONE AND VERIFIED (2026-09-13).** Full lifecycle (status/expiration/disable/re-enable/permanently-revoke/regenerate/version-supersession/quote-delete-cascade) shipped as migrations 137 (+141), 138, 139, 140 (+142), plus the Queue C2.6 internal controls UI | Expiry policy, revocation UX, and audit design all decided via `PRODUCT_SHARE_LINK_EXPIRATION_REVOCATION_DECISION.md` | None remaining for this batch — Queue C2.7 (direct-write closure, switching Create & Send to the server-owned RPCs) is tracked separately, not a business decision | N/A — done | Migration (done, prod-verified), Code (done, prod-verified), User acceptance pending real use |
