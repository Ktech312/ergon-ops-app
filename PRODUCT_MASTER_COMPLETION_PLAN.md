...
**Gate**: Every Critical-tier screen is keyboard-operable and screen-reader-passable (**6 of ~46
modals now have focus trap/return/Escape-close** via a new shared `useModalA11y` hook, live-verified
in a running instance; 40 remain; **12 of 12 flagged desktop clickable-rows are now
...
| Mobile/accessibility — modal focus management | **In progress** — `useModalA11y` reusable hook built and applied to 6 of ~46 modals (Purchasing: receive/edit/create-purchase; Inventory: add-or-edit-item, adjust; Projects: add-or-edit-BOM-line); live-verified in a running instance (Escape closes + returns focus, Tab/Shift+Tab trap holds) | Apply the same hook to the remaining ~40 modals, prioritizing the next tier of Inventory/Projects/Sales screens | High | Code (done for 6, remaining for ~40), User acceptance | None | Escape closes every Critical-tier modal; focus enters on open and returns to the trigger on close — met for 6, not yet for the rest |
...
| Sales presentation/templates (remaining items) | **Partially implemented** — misleading pricing wording, in-place BOM-line editing, branding wire-up, and mobile data-labels (§5 rows 2/3/8/9) are done; see §5 for the rest | Pricing display, `client_id`/`quote_ref` carry-through, approval gate, bundle explosion, searchable catalog picker, share-link expiry, and everything below §5 row 11 | Mixed — see §5 for per-batch priority | See §5 | Batch 1 (pricing) gates several later batches | One real deal closed through Ergon's own proposal flow, no PandaDoc/HubSpot step needed |
...
7. **Sales Batch 4 — `client_id` carry-through only** (High, small, safe — corrected this pass). `client_id` is a one-line addition to `create_project_from_quote()`'s existing insert list — the column already exists nullable on both `sales_quotes` and `projects` (migration `102_clients.sql`). **Note: this RPC is a stored Postgres function (migration 127), so even this "small" change is a migration** (`create or replace function`), not a frontend-only edit — corrected from the original wording, which implied pure application code. **`quote_ref` is NOT a one-line change as previously stated** — verified against `backend/supabase/migrations/066_sales_quote_ref_and_closed_at.sql` and `127_atomic_project_conversion.sql`: `quote_ref` exists only on `sales_quotes` (trigger-assigned, unique); `projects` has `source_sales_quote_id` (an FK-style id, already used for idempotency) but no human-readable reference column at all. Carrying it onto `projects` needs a new column plus a naming decision (e.g. `source_quote_ref` vs. reusing `quote_ref`) — deferred to Batch 4b below, not bundled into this small fix. *Verification (client_id only): Migration (small, additive, no unapplied-migration risk since it only extends an already-applied function), Prod verify.*
...
8. **Modal focus-trap/Escape-close reusable hook — PARTIALLY DONE.** `useModalA11y(isOpen, onClose)` built (Escape-close, Tab/Shift+Tab focus trap cycling within the modal, focus-return to the trigger element on close), matching this pass's `clickableRowProps` pattern for reuse over per-instance patching. Applied to 6 of the ~46 modals so far: Purchasing's receive-request/edit-request/create-purchase modals, Inventory's add-or-edit-item and adjust-inventory modals, and Projects' add-or-edit-BOM-line modal. Verified live in a running dev instance (unauthenticated/local-demo mode, no signed-in session available): Escape closes the modal and returns focus to the triggering button; Shift+Tab from the first focusable element wraps to the last, confirming the trap holds focus inside the modal. **40 modals remain unconverted** — this is real, bounded progress, not full coverage. *Verification: Code (done for the 6), User acceptance (remaining 40 + real authenticated use of the 6 already done).*
