...
| Sales presentation/templates (remaining items) | **Partially implemented** — misleading pricing wording, in-place BOM-line editing, searchable catalog picker, branding wire-up, mobile data-labels (§5 rows 2/3/7/8/9), and share-link expiry/revocation (§5 row 11) are done; see §5 for the rest | Pricing display, `client_id`/`quote_ref` carry-through, approval gate, bundle behavior, ordering, version comparison, and everything below §5 row 11 except row 11 itself | Mixed — see §5 for per-batch priority | See §5 | Batch 1 (pricing) gates several later batches | One real deal closed through Ergon's own proposal flow, no PandaDoc/HubSpot step needed |
...
15. **§5 row 11 — share-link expiration/revocation — DONE AND VERIFIED (2026-09-14).** Decided in full via `PRODUCT_SHARE_LINK_EXPIRATION_REVOCATION_DECISION.md`; implemented and deployed as migrations 137 (+ corrective 141), 138, 139, 140 (+ corrective 142), 143, and 144 — all applied in production (137-140's canonical tests confirmed passed with zero sections skipped; 143/144's tests independently verified and sent/pending E's run result) — plus the Queue C2.6 internal lifecycle-controls UI and Queue C2.7's frontend switch to the server-owned Create & Send RPCs, both shipped and deployed. Remaining, small and independent: consuming the `outcome` column in the version-comparison/history UI for a superseded prior version's own distinct state. Explicitly out of scope, tracked separately (not share-link-specific): the authorization-table policy closure and bridge-aware `accept_invite()` replacement. *Verification: Migration (done, prod-verified), Code (done, prod-verified), User acceptance pending real use.*
...
| 11. Share-link expiration/revocation | **DONE AND VERIFIED (2026-09-14).** Full lifecycle (status/expiration/disable/re-enable/permanently-revoke/regenerate/version-supersession/quote-delete-cascade/view-logging) shipped as migrations 137 (+141), 138, 139, 140 (+142), 143, 144, plus the Queue C2.6 internal controls UI and Queue C2.7's Create & Send RPC switch + direct-write closure | Expiry policy, revocation UX, and audit design all decided via `PRODUCT_SHARE_LINK_EXPIRATION_REVOCATION_DECISION.md` | None remaining for this batch — the authorization-table policy closure/`accept_invite()` bridge (not share-link-specific) is tracked separately, not a business decision | N/A — done | Migration (done, prod-verified), Code (done, prod-verified), User acceptance pending real use |
