...
| Sales presentation/templates (remaining items) | **Design-complete, not implemented** — see §5 Batches 2-9 | Pricing display, in-place BOM-line editing, `client_id`/`quote_ref` carry-through, approval gate, bundle explosion, branding wire-up, mobile data-labels, share-link expiry | Mixed — see §5 for per-batch priority | See §5 | Batch 1 (pricing) gates several later batches | One real deal closed through Ergon's own proposal flow, no PandaDoc/HubSpot step needed |
