...
| Mobile/accessibility — performance | **Most items done across this session's passes** — xlsx dynamic import (~113kB gzip win, confirmed in production bundle), Dashboard `activityFeed` memoized, `loadTasks` bounded to 2000, photo resize-before-upload now live on all three real photo-upload paths | `loadInventoryItems` deliberately left unbounded (alphabetical ordering means any cap would silently hide catalog items — a real pagination design is needed, not a mechanical limit) | Medium | Code (done except pagination), Business decision (pagination UX for `loadInventoryItems`) | None blocking | Every unbounded list load is either justified or paginated (not yet met — pagination decision pending); uploaded photos are resized client-side before upload — met |
...
| Sales presentation/templates (remaining items) | **Partially implemented** — misleading pricing wording, in-place BOM-line editing, searchable catalog picker, branding wire-up, and mobile data-labels (§5 rows 2/3/7/8/9) are done; see §5 for the rest | Pricing display, `client_id`/`quote_ref` carry-through, approval gate, bundle behavior, ordering, version comparison, share-link expiry, and everything below §5 row 11 | Mixed — see §5 for per-batch priority | See §5 | Batch 1 (pricing) gates several later batches | One real deal closed through Ergon's own proposal flow, no PandaDoc/HubSpot step needed |
...
| Client Ledger reliability | Logging is live; the unsafe blind-revert attempt was removed; Queue B1 completed the state-machine review and selected a per-project serialized latest-snapshot queue because it preserves existing workflow and prevents stale failure recovery from overwriting newer edits | Implement and ship `PRODUCT_CLIENT_LEDGER_SAVE_RECOVERY_PLAN.md` Option B under `CONTINUOUS_CODER_HANDOFF.md` A10 | **Critical** for Phase 7's own gate | Code, User acceptance | None | `updateProjectLedgerInfo` checked, logged, AND its caller safely surfaces/recovers from failure |
...
2. **Client Ledger save recovery — READY in continuous Queue A10** (Critical). Implement the reviewed per-project serialized latest-snapshot queue for `updateProjectLedgerInfo`; never blindly revert an older failed edit over a newer value. *Verification: Code, User acceptance.*
...
7. **Sales Batch 4 — `client_id` carry-through — MIGRATION DRAFTED, NOT RUN.** `backend/supabase/migrations/134_project_conversion_client_id_carry_through.sql` preserves `create_project_from_quote()`'s entire body byte-for-byte and adds `client_id` (already nullable on both `sales_quotes`/`projects` since migration `102_clients.sql`) to its insert list. Verification script `migration_134_client_id_carry_through_tests.sql` drafted alongside it (transaction-safe, proves with/without-client_id conversion, idempotent retry preserves client_id, and a non-admin/non-pm regression guard). **`quote_ref` is NOT a one-line change** — verified against `backend/supabase/migrations/066_sales_quote_ref_and_closed_at.sql` and `127_atomic_project_conversion.sql`: `quote_ref` exists only on `sales_quotes` (trigger-assigned, unique); `projects` has `source_sales_quote_id` (an FK-style id, already used for idempotency) but no human-readable reference column at all. Carrying it onto `projects` needs a new column plus a naming decision (e.g. `source_quote_ref` vs. reusing `quote_ref`) — deferred to Batch 4b below, not bundled into this fix. *Verification (client_id only): Migration (drafted), Prod verify (pending E's review/run).*
...
15. **§5 row 11 — share-link expiration/revocation** (High, security-adjacent). A column already exists and nothing ever sets it. *Verification: Business decision (expiry policy), Code.*
...
| 4. `client_id` carry-through | Add `client_id` (already a nullable column on both `sales_quotes` and `projects`, migration `102_clients.sql`) to `create_project_from_quote()`'s insert list — this RPC is a stored Postgres function, so the change itself is a migration (`create or replace function`), not application code, even though it's small | Yes — small, scoped data-completeness fix to an already-hardened, already-shipped atomic function | None | High | Migration (small), Prod verify |
...
| 10. Reorder for BOM lines — DONE (quote BOM lines); proposal sections remain | Built as accessible Move up/Move down buttons (not a drag-and-drop library) writing back to `sales_quote_bom_lines.line_sort`, which existed unused until now. Proposal Template sections (`sequence_order`) not yet done — see Queue A4 | Yes — pure UI/UX, no schema change needed | None | N/A for quote BOM lines — done; Medium for proposal sections | Code (done for quote BOM lines) |
...
| 12. Version-comparison view — DONE | Read-only `compareProposalSnapshots()` + a "Compare versions" UI in the Quote Proposal section, defaulting to the newest two versions with explicit selectors for any pair | Yes — self-contained, no approval/process implications | None | N/A — done | Code (done); User acceptance pending real multi-version proposal data (no real quote has a sent proposal yet) |
