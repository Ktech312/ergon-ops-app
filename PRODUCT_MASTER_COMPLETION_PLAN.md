...
10. **xlsx replacement decision + migration to `exceljs`** (High, decision-gated). Every other dependency advisory `npm audit` found is now cleared (Queue A13); this is the one that genuinely needs the decision, not just a version bump. *Verification: Business decision, then Code, Prod verify.*
