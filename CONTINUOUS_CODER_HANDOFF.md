...
**Manual-action queue, current exact state (superseded again since -- see the 2026-09-17 Phase 3
entries further below for the live state)**: migrations 152/153/154 all DONE, no longer queued. Do not
re-litigate D5/D6/D8/D9/D12/D18 (table further above) or rerun any of them.
...
rolled-back transaction. **Migration 155 CONFIRMED APPLIED and its canonical test PASSED in production
(2026-09-17)** -- the test's own Section 5 had a real fixture-ordering bug (tried to insert a fresh
row as a caller already moved to the suspended workspace; `guard_workspace_id_mutation()` correctly
blocks every write, including INSERT, for a suspended-workspace caller, so the fixture creation itself
failed before the read/write assertions could run) -- found on E's first live run, fixed same-day
(`3a7692a`, test script only, migration 155 itself never touched), re-run clean. No longer in the
manual-action queue. Zero visible production change observed, as predicted (exactly one real
workspace).
...
stage boundaries (a table having an FK to `projects` does not automatically make it "Projects" stage
if its own domain is named as a LATER stage) -- `project_documents` stays Stage 4, the four
...
template library), `project_ref_counters` stays Stage 5 (a shared counter table). `tasks.workspace_id`
derives from each task's own creator's workspace membership rather than fuzzy-matching the loose
`project_ref` text column (`tasks` has no real FK to `projects`) -- a mechanical extension of the
existing ownership-trigger pattern, judged a routine implementation default rather than a business
decision needing E's input. **Migration 156 CONFIRMED APPLIED and its canonical test PASSED in
production (2026-09-17, "Success. No rows returned" for both).** No longer in the manual-action queue.

**Stage 2's RLS half delivered**: migration 157 (`faee9d6`) adds workspace-scoped RLS to the 14-table
Projects/Tasks ownership graph, following migration 156 the way migration 155 followed 117.
Preserves every existing access rule found by direct read: `projects`/`project_scope_of_work`/
`project_bom_lines` keep their pm/admin role-gated write policies (migration 023), ANDed with the
workspace check; `project_submittals` stays SELECT-only (RPC-only writes since migration 144);
`task_activity_log` keeps its append-only shape; `project_conversion_receipts` is left completely
untouched (already maximally locked down by permanent design, migration 127). Hardens 3
security-definer RPCs with zero caller-workspace check (`create_and_send_submittal_version`,
`get_submittal_by_token`, `respond_to_submittal` -- the latter two also gain the suspended-workspace
check). Unlike migration 155's RPCs, no `active_workspace_id()` landmine here -- these two RPCs are
submittal-only, not shared with proposals, so their settings lookups use the target project's own
resolved `workspace_id` directly. **Migration 157 CONFIRMED APPLIED and its canonical test PASSED in
production (2026-09-17)** -- the test needed two same-day fixes on E's first two live runs: a `RAISE`
call missing its argument plus a structural check querying a nonexistent `cmd='ALL'` policy
(`fd39b66`), then a genuine fixture-ordering bug where `create_and_send_submittal_version`'s own real
supersession side effect (marking any other active token for the same project as `superseded` when a
new version is sent) flipped `token_a`'s own status before a later suspended-workspace section could
run against it (`ad18ef8`) -- migration 157 itself was never touched by either fix. **Stage 2
(Projects/Tasks -- ownership + RLS) is now fully shipped end-to-end.**
...
`regenerate_share_link`, `create_and_send_quote_proposal_version`) -- both `projects.workspace_id`
and `sales_quotes.workspace_id` are live, so this cleanup that migrations 155/157 both deliberately
deferred could finally ship. While fixing each, found and fixed a second, more serious gap in the
same pass: all 7 share-link lifecycle RPCs checked the caller's role but never the caller's workspace
against the target entity's own workspace -- two of the seven (`create_submittal_share_token`,
`create_quote_proposal_share_token`) were directly callable by any authenticated PM/admin or
Sales/manager/admin with zero workspace check at all, even though the current frontend no longer
calls either one. Two new shared helpers (`share_link_entity_workspace_id`,
`assert_share_link_in_caller_workspace`) close this for all 7, alongside their existing role checks.
`active_workspace_id()` itself remains in use by the legacy admin-role bridge and by
`save_equipment_recipe()`/`replace_project_bom_lines()` (`equipment_types` has no `workspace_id` yet)
-- genuinely still needed there. **Migration 158 CONFIRMED APPLIED and its canonical test PASSED in
production (2026-09-17, "Both - Success. No rows returned").** Phase 3's cross-cutting cleanup is now
...
`build_transactions`), mirroring migration 156's ownership-then-RLS pattern exactly. Scope confirmed
by a dedicated research pass grepping every reference to each table across all 158 prior migrations:
`purchase_order_lines`/`_files`/`_receipts`/`_holds`, `inventory_balances`, `inventory_movements`,
`inventory_transactions`, `project_inventory_allocations`, `project_allocation_history`, and
`equipment_bom_components` all inherit ownership through their required FK (no new column), same as
Stage 2's children. Confirmed via the same pass: unlike Sales/Clients/Share-links, this domain has
almost no RPC layer at all -- every write except `save_equipment_recipe()` (equipment recipes) and
the read-only `replace_project_bom_lines()` (Stage 2's own guard, not this one) goes directly through
PostgREST gated only by RLS, so this stage's containment work is almost entirely an RLS/column
exercise. Migration 159 also retires `save_equipment_recipe()`'s `active_workspace_id()` fail-closed
guard -- the exact retirement migration 158 predicted as pending here -- replacing it with real
per-caller workspace containment on every lookup the function does (existing-recipe resolution by id
or name, component name resolution, output-item resolution), verbatim-carried-forward from the real
migration 130 body with five targeted edits, not a rewrite from memory. RLS on all seven root tables
(and their children) is deliberately untouched by 159 -- that is migration 160, next. **Migration 159
CONFIRMED APPLIED and its canonical test PASSED in production (2026-09-17)** -- one same-day
test-script fix, `e75306a`: the fixture vendor created to reach the `purchase_orders` no-membership
guard needed a real caller identity, not the `postgres` role -- `guard_workspace_id_mutation()` is a
BEFORE INSERT trigger, not an RLS policy, so switching to `postgres` bypasses RLS but not trigger
execution; migration 159 itself was never touched. **Stage 3's ownership half is now fully shipped.**

**Stage 3's RLS half delivered**: migration 160 (`df341ab`) adds workspace-scoped RLS to the seven
root tables plus their ten children (`purchase_order_lines`/`_files`/`_receipts`/`_holds`,
`inventory_balances`, `inventory_movements`, `inventory_transactions`,
`project_inventory_allocations`, `project_allocation_history`, `equipment_bom_components`), mirroring
migration 157's ownership-then-RLS pattern exactly. Three new owner-resolver helper functions
...
for `project_inventory_allocations`. Two tables (`inventory_transactions`, `project_allocation_history`)
have no single NOT NULL anchor column, resolved inline via a coalesce of two or three resolvers rather
than a dedicated function. Every pre-existing role gate from migration 023
(warehouse/purchasing/admin) is preserved, ANDed with the new workspace predicate -- confirmed
`vendors`/`locations`/`purchase_orders` and their four immediate children never had a role gate at
all (absent from 023's own list, still `using(true)` today), unlike their purchasing/warehouse-adjacent
siblings. No RPC changes -- confirmed this domain has no RPC layer besides `save_equipment_recipe()`,
already hardened by 159. `purchase_order_files`' own storage.objects bucket policies are deliberately
untouched -- Stage 4 territory (storage). **Migration 160 CONFIRMED APPLIED and its canonical test
...
- **176** (`3912b92`) -- `product_catalog` (`catalog_number` uniqueness now per-workspace) +
  `catalog_price_change_requests` (workspace_id derived from its mandatory parent via a new guard
  trigger). Test-file bug fixed: simulated JWT was missing the email claim the table's RLS checks.
- **177** (`7e99325`) -- extends migration 173's "each company gets its own copy" ruling to
  `presales_hardware_rules`/`site_hardware_rules`/`form_schemas`+`form_schema_fields`; also fixes
  `derive_deletion_log_workspace_id()`'s stale "genuinely global" classification of these entity types.
  Two test-file bugs fixed: a CHECK constraint value, and column-name mismatches (`name`/`title`,
  `label`+`sequence_order`/`field_label`+`display_order`).
- **178** (`bbe014d`) -- `one_off_reconciliations` (flat inventory-merge audit log, previously
...
two `deletion_log` entity types that are genuinely hard-deleted, so their `workspace_id` used to stay
null forever. Three new atomic RPCs capture `workspace_id` before deleting the row, in the same
transaction, preserving every existing client behavior exactly. Independently verified end-to-end
against a real local PostgreSQL 18 engine (PGlite), including a workspace-scoped-sku ambiguity
scenario. **A real process mistake this session**: the matching `src/persistence.ts` commit (`5a5f919`)
was committed with the explicit intent to hold it back until this migration was confirmed applied --
but `git push` sends every commit ahead of `origin/main`, not just the one just created, so it went out
anyway with a later, unrelated doc-reconciliation push, deploying it BEFORE E had run migration 172.
Real impact: a window where every inventory-item/equipment-type delete attempt in the live app would
have failed (the RPC didn't exist yet) -- no data corrupted, but a real live-functionality gap, caught
after the fact via the deployed bundle hash. The app is back in a consistent, working state as of
2026-09-18. Lesson for future sessions: a commit that must wait for a dependency should not be
committed to `main` at all until that dependency is confirmed -- "committed but not pushed yet" is not
...
- **`notification_rules` is very likely Stage 5's job, not Stage 4's** -- a global, admin-configured
  ...
  156's own header flagged this overlap. Worth confirming with E, though it doesn't block anything
  else in Stage 4.
- **"Jobs" does not exist anywhere in this codebase** -- confirmed via repo-wide grep (no queue, cron
  table, or webhook log). If any earlier planning doc assumed one, that assumption is now corrected.
- `active_workspace_id()` confirmed fully retired from this entire domain -- no new call sites.
- Cross-cutting, not owned by this stage: `deletion_log` (polymorphic audit table, no anchor, logs
  deletions from several Stage-4-adjacent tables) needs its own decision on which pass fixes it;
  `app_sync_events`/`app_transaction_locks` are pre-`workspaces`-feature MVP leftovers, a cleanup item
  not part of containment work.
...
exist in the WHOLE DATABASE -- used by several RPCs (`replace_project_bom_lines`, the legacy
`bridge_*` admin-role functions, and the share-link RPCs' SUBMITTAL-side branches) precisely because
the tables THEY touch (`projects`, `project_submittals`) don't have their own `workspace_id` yet, or
(`replace_project_bom_lines`) the guard is incidental to a table already scoped. `save_equipment_recipe`
was the one domain-relevant call site and is now retired (migration 159). Not a bug to fix once --
retire the rest incrementally as each later stage above gives its own table group real per-row
workspace ownership.
