...
Stage 2's RLS half (the access-restriction migration for `projects`/`tasks` and their child tables,
mirroring migration 155 for Clients+Sales) is not yet written -- next automatic task.

**Manual-action queue, current exact state: EMPTY.** Migrations 155 and 156 are both confirmed
