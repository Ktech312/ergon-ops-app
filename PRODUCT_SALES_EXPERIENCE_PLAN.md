...
- **Usability friction**: A BOM line, once added, cannot be edited — only its catalog link can (`onUpdateBomLineCatalogLink`, `main.tsx:22745`). There is no `updateSalesQuoteBomLine` function in `persistence.ts` for item name/qty/notes; fixing a typo or quantity means delete (`onDeleteBomLine`, `main.tsx:22754`) and re-add, which also discards any notes that were on that line.
- **Suggested improvement**: Add an in-place edit affordance for item/qty/notes on an existing BOM line (small, surgical — the row already renders those three fields, it just needs inputs instead of static text plus a save action). Separately, a "start from a template" or "duplicate an existing quote" action would remove the blank-page cost on every new deal — flagged as a real gap by both this pass and `PRODUCT_SALES_DISCOVERY.md` §3.1/§4.
