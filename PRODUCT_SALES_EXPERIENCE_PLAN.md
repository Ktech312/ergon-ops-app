...
- **Current capability**: **Done and deployed.** The proposal BOM table carries `className="proposal-bom-table stack-table-mobile"`, and every data cell now carries the matching `data-label` (`Image`, `Item`, `Description`, `Qty`, `Datasheet`). The existing phone-width stacking rule therefore retains the meaning previously supplied only by the hidden desktop header.
- **Remaining capability**: Real-device/customer acceptance and the broader 360/390/768/print verification pass in `CONTINUOUS_CODER_HANDOFF.md` A6. The structural missing-label defect itself is closed.
- **Needs a product decision?**: No.

### Brand customization

- **Current capability**: **Done and deployed for the existing company name/logo.** New proposal snapshots freeze `companyName` and `companyLogoUrl` at send time, and `ProposalPublicPage` renders that frozen identity. Later Admin branding edits do not retroactively alter a proposal already sent to a customer. Older snapshots without the fields retain a compatible fallback.
- **Remaining capability**: Per-deal/per-template colors or themes are still a separate decision. Productization still requires workspace-scoped branding once real multi-workspace onboarding is allowed.
- **Needs a product decision?**: No for the delivered name/logo behavior. Yes for themes beyond it.
...
- **Suggested improvement**: Add `client_id` to `create_project_from_quote()`'s insert list as a small migration that preserves the entire latest live function. Treat `quote_ref` separately: `projects` has no human-readable destination column, so it requires a new-column/name decision rather than being folded into the `client_id` change.
- **Needs a product decision?**: No for `client_id`. Yes for whether to add `source_quote_ref` (or another named field) to Projects.
...
2. **Continue the small authoring/conversion closeouts.** In-place BOM-line editing and the searchable catalog picker are already deployed. Next are accessible ordering controls and the `client_id` carry-through migration package. Human-readable `quote_ref` carry-through is a separate new-column decision, not part of the small `client_id` change.
...
Lower than pricing but safe to continue independently: ordering polish and a version-comparison view. The searchable catalog picker, company-name/logo branding, and mobile proposal labels are already deployed. Per-template themes remain a later product decision.
