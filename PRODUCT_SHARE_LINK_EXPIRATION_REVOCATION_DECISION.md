...
1. **Drift report first.** Standalone, read-only SQL (`backend/supabase/drift_report_standalone.sql`) prepared, covering all six required categories, runnable immediately against production — independent of migration 124, which does not need to exist for it to work. **Historical drafting state:** it had not yet been run at this point in the design sequence. It was subsequently run clean, and the bridge was later fully tested, deployed, and live-verified; see Part 13.5 and the closing status below. Discrepancies are reported, never auto-resolved.
...
Status: **design-only and still unimplemented.** The original reason for holding it — pending migration 124 verification — has since been resolved: migrations 124-126 are tested, deployed, and live-verified. The policy-closure plan itself has not been turned into a runnable migration or applied, because beginning that RLS/policy change remains a separate decision and approval checkpoint.
...
Full corrected `migration_124_bridge_tests.sql` delivered to E (sent as a file, not pasted into chat — chat pasting of the full 1000+ line script made it hard to scroll back to instructions; file delivery is now the default for any full-file handoff). **Historical state at delivery: not yet run. It was subsequently run clean; see Part 13.5 immediately below.**
...
**Migration 125 (13.1) is run and verified — closed. Migration 126 (13.3) is run and applied. Gate 1 (migrations 124-126, the workspace-authorization bridge) is fully closed** — committed, pushed, deployed to production, and live-verified with zero drift; see HANDOFF.md's 2026-09-09 entry. **Migrations 127 and 128 (the atomic Sales Quote -> Project conversion rewrite, plus its INSERT-trigger-chain hardening) are both applied in Supabase and fully closed** — `migration_127_conversion_tests.sql` was rerun against both migrations together and passed with zero skips, and the full change set is committed, pushed, and deployed to production as commit `9e8ce64`; see HANDOFF.md's 2026-09-10 entries. Migration 129's policy-closure plan (Part 12) is still design-only.
