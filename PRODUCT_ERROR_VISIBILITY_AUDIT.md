...
**`createProjectFromClosedWonQuote` — critical, partial multi-step write with no rollback.** `persistence.ts:10102-10311`. After the top-level `projects` insert succeeds (checked, surfaces failure via `window.alert` at `main.tsx:22299-22311`), four more write categories fire with **no `.ok` check at all**: `project_scope_of_work` insert (10170), `project_bom_lines` insert (10187), `project_location_items` insert (10238), `project_location_images` insert (10259, with a silent-retry-without-`origin`-column fallback at 10279). The one checked sub-write (`project_locations`, 10206) `console.error`s and `continue`s past a failed location. **Employee sees**: "Created Project X" success message; the new project may be missing BOM lines, scope of work, or photos with zero indication. BOM lines feed procurement — a silently-missing line means hardware isn't ordered, or is ordered short, discovered only in the field. **Severity: Critical.** **Recovery**: track per-step success/failure, surface "created, but N of M lines/photos didn't copy — review before starting the build," add a scoped retry for just the failed sub-writes.
