...
- **Unresolved OPTIONAL associations: existing behavior preserved this pass, permitted by the current schema, NOT yet confirmed as the desired product rule — a documented future data-quality decision, corrected wording 2026-09-11.** `saveBuildTransactions`' unmatched equipment name, and `saveProjectAllocations`'/`saveInventoryMovements`' unmatched project/build names, all still resolve silently to `null`, exactly as they did before this pass — that part is unchanged. **What changed is only the claim about WHY**: an earlier draft of this section described this as "documented behavior" and "schema-valid, documented" as if a nullable FK settles the question. It doesn't — a nullable FK proves only that the DATABASE will accept a null value here; it says nothing about whether saving a build/movement/allocation with a silently-unmatched name is what the product actually wants (versus, say, rejecting it the same way an unresolved sku now is, or flagging it for review). This pass makes no such decision and changes no such behavior — it is named here explicitly as an open question for E, not resolved by omission or by citing the schema as if that were an answer.
...
**3. Retention period.** Resolved rows are retained for **90 days** after their `resolved_at` timestamp, then eligible for deletion — long enough to answer "has this happened before" during a later incident review (a real, recurring need this session's own multi-day audit trail has repeatedly relied on) without keeping an unbounded table. Active/Acknowledged rows are never auto-deleted regardless of age — an old *unresolved* row aging out silently would be worse than the original visibility gap this whole feature exists to close. 90 days is a starting recommendation, not derived from any existing policy in this codebase (none exists to anchor it to) — flagged as a number E should confirm, not treated as settled.
